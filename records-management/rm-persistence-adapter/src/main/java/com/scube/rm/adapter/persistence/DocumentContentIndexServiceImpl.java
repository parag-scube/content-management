...
public class DocumentContentIndexServiceImpl implements IndexService<DocumentJpaEntity> {
	
	private static final Log LOGGER = LogFactory.getLog(DocumentContentIndexServiceImpl.class);
...
//    }
    ...
    @Autowired
    public DocumentContentIndexServiceImpl(RestHighLevelClient client, IndexManager manager) {
...
//    	        this.renditionService = renditionService;
	...
	@Override
	public void index(DocumentJpaEntity document, InputStream content) {
...
//                "}";
	...
	@Override
	public void unindex(DocumentJpaEntity document) {
		if (!pipelinedInitialized) {
            try {
                ensureAttachmentPipeline();
            } catch (IOException ioe) {
